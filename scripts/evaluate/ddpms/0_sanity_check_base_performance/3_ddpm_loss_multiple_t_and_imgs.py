...
if __name__ == '__main__':
    ...
    for i in tqdm(range(args.num_iterations)):
        ...
        for t in noise_timesteps:
            ...
            for img_mismatch, seg_mismatch in mismatch_dl:    
                img_mismatch = img_mismatch.to(device)
                seg_mismatch = seg_mismatch.to(device)
                
                b = img_mismatch.shape[0]
                
                # Calculate the ddpm_loss
                t_tch = torch.full((b,), t, device=device).long()
                img_mismatch = ddpm.normalize(img_mismatch)
                seg_mismatch = ddpm.normalize(seg_mismatch)
                
                ddpm_loss_sample += (1 / len(mismatch_dl)) * ddpm.p_losses_conditioned_on_img(img_mismatch, t_tch, seg_mismatch)
