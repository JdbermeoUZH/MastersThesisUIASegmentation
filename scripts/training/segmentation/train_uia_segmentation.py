...
from dataset.dataset_h5 import get_datasets
...
def preprocess_cmd_args() -> argparse.Namespace:
    ...
    # Training loop
    # -------------:
    parser.add_argument('--fold', type=int, help='Fold to train on. Default: 0')
    parser.add_argument('--train_split', type=str, help='Name of split to use for training. Default: train')
    parser.add_argument('--val_split', type=str, help='Name of split to use for validation. Default: test')
...
if __name__ == '__main__':
    ...
    fold                = train_config['segmentation']['fold']
    ...
    h5_filepath         = dataset_config[dataset]['preprocessed']['h5_filepath']
    ...
    # Model definition
    norm = Normalization(
        n_layers        = model_config['normalization_3D']['n_layers'],
        image_channels  = model_config['normalization_3D']['image_channels'],
        channel_size    = model_config['normalization_3D']['channel_size'],
        kernel_size     = model_config['normalization_3D']['kernel_size'],
        activation      = model_config['normalization_3D']['activation'], 
        batch_norm      = model_config['normalization_3D']['batch_norm'],
    ...
    seg = UNet(
        ...
        n_classes               = n_classes,
        channels                = model_config['segmentation_3D']['channel_size'],
        channels_bottleneck     = model_config['segmentation_3D']['channels_bottleneck'],
        skips                   = model_config['segmentation_3D']['skips'],
