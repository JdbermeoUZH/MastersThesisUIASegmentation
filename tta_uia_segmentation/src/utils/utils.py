...
def get_generators(num_generators=1, seed=None, library="torch"):
    ...
    generators = [get_generator(seed, library=library) for _ in range(num_generators)]
    return generators
...
def random_select_from_tensor(tensor, dim):
    assert isinstance(
        tensor, torch.Tensor
    ), f"tensor is of type {type(tensor)} but should be torch.Tensor"
...
def resize_volume(
    ...
    # :===============================================:
    assert len(input_format) == len(x.shape), (
        f"x has {len(x.shape)} dimensions ({x.shape}) "
    ...
    if target_size_before_resample is not None:
        crop_or_pad_necessary = tuple(x.shape[-3:]) != tuple(
            target_size_before_resample
        )
        x = crop_or_pad_to_size(x, target_size_before_resample)

    # Resample/resize volume
    # :===============================================:
    scale_factor = torch.Tensor(current_pix_size) / torch.Tensor(
        target_pix_size
    )  # (D, H, W)
    resampled_size = (
        (torch.tensor(x.shape[-3:]) * scale_factor).round().int().tolist()
    )  # (D, H, W)

    if only_inplane_resample:
        if target_img_size is not None:
            assert (
                x.shape[-3] == target_img_size[0]
            ), "Target depth must match depth of 'x' for only_inplane_resize=True"
        assert mode in [
            "bilinear",
            "nearest",
        ], "Only 'bilinear' and 'nearest' modes are supported for only_inplane_resample=True"
    ...
    # :===============================================:
    if target_img_size is not None:
        crop_or_pad_necessary = tuple(x.shape[-n_spatial_dims:]) != tuple(
            target_img_size[-n_spatial_dims:]
        )
...
def resize_image(
    ...
    # :===============================================:
    assert len(img_format) == len(x.shape), (
        f"x has {len(x.shape)} dimensions ({x.shape}) "
    ...
    # :===============================================:
    if target_size_before_resample is not None:
        crop_or_pad_necessary = tuple(x.shape[-2:]) != tuple(
            target_size_before_resample
        )
        x = crop_or_pad_to_size(x, target_size_before_resample)
...
def resize_and_resample_nibp(
    ...
    if len(image.shape) == 3:
        return nibp.conform(
            nib.Nifti1Image(image, affine_matrix),
            target_size,
            target_voxel_size,
            order=order,
        ).get_fdata()
    ...
    for channel_i in range(image.shape[0]):
        ...
        new_img_channels.append(
            nibp.conform(
                nib_img, target_size, target_voxel_size, order=order
            ).get_fdata()
        )
...
def crop_or_pad_to_size(
    data: torch.Tensor, target_size: Union[Tuple[int, int], Tuple[int, int, int]]
) -> torch.Tensor:
    """
    ...
    padded_cropped = F.pad(
        data, [item for sublist in reversed(pad_crop) for item in sublist]
    )

    assert tuple(padded_cropped.shape[-len(target_size) :]) == tuple(
        target_size
    ), f"Output shape {padded_cropped.shape} does not match target size {target_size}"
...
def stratified_sampling(
    a, b, m, n, return_torch: bool = True
    ...
    if n < m:
        raise ValueError(
            "Warning: Number of samples is less than number of groups to stratify on"
        )

    bucket_size = (b - a) / (m)
    buckets = np.arange(a, b + 1, bucket_size)
    buckets = [
        (np.round(buckets[i]), np.round(buckets[i + 1]))
    ...
    for draw_n in draws:
        ...
        sampled_t.append(np.random.randint(bucket_start, bucket_end, (draw_n,)))
...
def generate_2D_dl_for_vol(
    *vols: torch.Tensor, batch_size: int, num_workers: int, **dl_kwargs
    ...
    **kwargs
    ...
    processed_vols = []
    for x in vols:
        processed_vols.append(from_NCDHW_to_DCHW(x))

    # Ensure all tensors have the same number of slices
    assert all(
        t.size(0) == processed_vols[0].size(0) for t in processed_vols
...
def numpy_to_torch(*arrays: np.ndarray) -> torch.Tensor | tuple[torch.Tensor, ...]:
    """
    ...
    *arrays : np.ndarray
    ...
    -------
    torch.Tensor
        PyTorch tensor representation of the input arrays.
    """
    if len(arrays) == 1:
...
def min_max_normalize_channelwise(
    tensor: torch.Tensor, spatial_dims: tuple[int, ...], eps=1e-8
) -> torch.Tensor:
    ...
    normalized_tensor = (tensor - min_vals) / (
        max_vals - min_vals + eps
