...
from tta_uia_segmentation.src.models.ddpm.UNetModelOAI import (
    UNetModelConditionedOnSegMask,
)
...
class ConditionalGaussianDiffusionOAI(torch.nn.Module):
    ...
    def forward(self, img, cond_img):
        ...
        losses = self.ddpm.training_losses(
            self.model, img, t, model_kwargs={"x_cond": cond_img}
        )
    ...
    def p_sample_loop(self, img_shape, x_cond, **kwargs):
        return self.ddpm.p_sample_loop(
            self.model, img_shape, model_kwargs={"x_cond": x_cond}, **kwargs
        )
