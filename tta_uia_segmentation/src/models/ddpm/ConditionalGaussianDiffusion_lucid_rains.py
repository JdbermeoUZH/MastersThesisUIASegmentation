...
class ConditionalGaussianDiffusion(GaussianDiffusion):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not only_unconditional:
            ...
            ), "Unet model must be defined in conditional mode"

    def model_predictions(
        self, x, t, x_cond, clip_x_start=False, rederive_pred_noise=False
    ):
    ...
    def p_losses_conditioned_on_img(
        self,
        x_start,
        t,
        x_cond,
        ...
        if not self.only_unconditional:
            assert (
                x_cond.shape[-2:] == x_start.shape[-2:]
            ), "x_cond and x_start must have the same Height and Width"
        ...
        if w_clf_free > 0:
            assert (
                self.also_unconditional
            ), "w_clf_free is only available when also_unconditional is True"
            assert (
                not self.only_unconditional
            ), "w_clf_free only makes sense when the model was trained in conditional and unconditional mode"
            x_unconditional = self._generate_unconditional_x_cond(
                batch_size=x_cond.shape[0], device=x_cond.device
            )
            ...
            model_out = (
                model_out - w_clf_free / (1 + w_clf_free) * model_out_unconditional
            )
    ...
    def distillation_loss(
        ...
        **kwargs,
        ...
        if not self.only_unconditional:
            ...
            ), "cond_img and img must have the same Height and Width"

        img = self.normalize(img)
        if img.max() > 1 or img.min() < -1:
            print(
                "Warning: img is not normalized between -1 and 1"
        ...
        if cond_img.max() > 1 or cond_img.min() < -1:
            print(
                "Warning: cond_img is not normalized between -1 and 1"
        ...
        if type == "sds":
            ...
            return self.posterior_distillation_sampling(
                img, t, cond_img, *args, **kwargs
            )
        else:
    ...
    def score_distillation_sampling(
        ...
        with torch.inference_mode():
            ...
            if w_clf_free > 0:
                ...
                model_preds_unconditional = self.model_predictions(
                    x_t, t, x_unconditional
                )
                pred_noise = (
                    1 + w_clf_free
                ) * pred_noise - w_clf_free * model_preds_unconditional.pred_noise
    ...
    def delta_denoising_score(
        self,
        x_start,
        t,
        x_cond,
        sampled_noise_ref: tuple[torch.Tensor],
        w_clf_free: float = 0,
    ):
        raise NotImplementedError("delta_denoising_score is not implemented yet")

    def posterior_distillation_sampling(
        self,
        x_start,
        t,
        x_cond,
        sampled_x_ref: tuple[torch.Tensor],
        w_clf_free: float = 0,
    ):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.only_unconditional:
            assert (
                cond_img is None
            ), "cond_img must be None when only_unconditional is True"
        ...
        if self.also_unconditional and self.unconditional_rate > 0:
            ...
            if self.also_unconditional:
                assert (
                    cond_img.shape[1] > 1
                ), "cond_img must be one hot encoded if training a single model in conditional and unconditional mode"
                # Choose randomly whether it will be a conditional or unconditional forward pass
                if random.random() < self.unconditional_rate:
                    cond_img = self._generate_unconditional_x_cond(
                        batch_size=cond_img.shape[0], device=cond_img.device
                    )
                    pixel_weights = None
        ...
        return self.p_losses_conditioned_on_img(
            img, t, cond_img, pixel_weights=pixel_weights, *args, **kwargs
        )
    ...
    def ddim_sample(self, img_shape, x_cond, return_all_timesteps=False):
        ...
        for time, time_next in tqdm(time_pairs, desc="sampling loop time step"):
            ...
            pred_noise, x_start, *_ = self.model_predictions(
                img, time_cond, x_cond, clip_x_start=True, rederive_pred_noise=True
            )
    ...
    def sample(
        ...
        if not self.only_unconditional:
            assert (
                img_shape[-2:] == x_cond.shape[-2:]
            ), "img_shape and x_cond must have the same shapes"

            if unconditional_sampling:
                assert (
                    self.also_unconditional
                ), "unconditional_sampling is only available when also_unconditional is True"
                assert x_cond.shape[1] > 1, "cond_img must be one hot encoded"

                x_cond = self._generate_unconditional_x_cond(
                    batch_size=x_cond.shape[0], device=x_cond.device
                )
    ...
    def _generate_unconditional_x_cond(self, batch_size: int, device: str):
        if self.also_unconditional:
            ...
            raise ValueError(
                "DDPM must be in unconditional. Either in `also_unconditional` or `only_unconditional` mode"
            )
