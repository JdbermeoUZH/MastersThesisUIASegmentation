...
class ConditionalLatentGaussianDiffusion(BaseConditionalGaussianDiffusion):
    ...
    def forward(
        ...
        noised_img_latents = self._noise_scheduler.add_noise(img_latents, noise, t)
        ...
        if self._forward_type == "train_loss":
            output_dict["loss"] = self._train_loss(img_latents, model_output, t, noise)
    ...
    def _apply_conditioning_to_latents(
        self, img_latents: torch.Tensor, cond_img_latents: torch.Tensor
    ) -> torch.Tensor:
    ...
    def _set_mixed_precision_attributes(
...
class PosEmbed2D(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.norm = (
            torch.nn.LayerNorm(embed_dim, elementwise_affine=False, eps=1e-6)
            if layer_norm
            else None
        )
        ...
        pos_embed = self._calculate_pos_embed(
            usual_height, usual_width, force_recalc=True
        )
    ...
    def _calculate_pos_embed(self, height, width, force_recalc=False):
        if height != self.usual_height or width != self.usual_width or force_recalc:
            grid_size = (height // self.patch_size, width // self.patch_size)

            extra_kwargs = (
                dict(crops_coords=((0, 0), grid_size))
                if self.pos_embed_type == "rotary"
                else dict()
            )

            pos_embed = self.pos_emb_fn(
                self.embed_dim, grid_size=grid_size, **extra_kwargs
            )
